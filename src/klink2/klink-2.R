...
infer <- function(x, y) {
    ...
    # for each relation holds {-1, 0, 1} for
    ...
    for (r in 1:rn) {
        ...
        if (h >= tR[r]) {
            hierarchy <- c(hierarchy, 1)
        } else if (h <= -tR[r]) {
            hierarchy <- c(hierarchy, -1)
        } else {
            hierarchy <- c(hierarchy, 0)
        }
...
klink2 <- function(input_file_name, export = FALSE) {
    ...
    inputfile <- paste(data_dir, input_file_name, "/", input_file_name, ".Rdata", sep = "")
...
export_triples <- function(output_file_name) {
    df_to_export <- triples %>%
        mutate(
            relation = case_when(
                relation == 1 ~ paste("klink", semantic[1], sep = ":"),
                relation == 2 ~ paste("klink", semantic[2], sep = ":"),
                relation == 3 ~ paste("klink", semantic[3], sep = ":"),
                relation == 4 ~ paste("klink", semantic[4], sep = ":"),
                TRUE ~ as.character(relation) # Keep other values as is
            )
        )
    write.table(
        df_to_export,
        paste(data_dir, output_file_name, "/", output_file_name, "_triples.csv", sep = ""),
        sep = ";",
        row.names = FALSE,
        quote = FALSE
    )
