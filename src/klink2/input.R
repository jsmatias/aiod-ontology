...
read_dataset <- function(limit = -1, named_list) {
  ...
  # fields used: 2 keywords fields, document title, authors,
  # publication name, research areas, year
  # NOTE: these fields are already checked for empty in case of solar dataset,
  # but must be tailored for another dataset
  # Changed 2 keywords fields to a single one.
  # d <- d[c("DE", "ID", "TI", "AU", "SO", "SC", "PY")]
  d <- d[c("DE", "TI", "AU", "SO", "SC", "PY")]
  ...
  process_item <- function(item) {
    ...
    a <<- a + 1
    # keywords <- unique(unlist(
    #   sapply(strsplit(tolower(item[c("DE", "ID")]), ";"), trimws)
    # ))
    keywords <- unique(unlist(
      sapply(strsplit(tolower(item["DE"]), ";"), trimws)
    ))
...
run_all <- function(limit = -1, named_list) {
  # just in case
  keywordsdb <<- new.env(parent = globalenv(), hash = TRUE)
  reldb_df <<- list()
  reldb_l <<- list()
...
inspect_dataset <- function(filename) {
  ...
  for (i in 1:rn) {
    cat(
      "Value of co-occurrence,", relations[i], "relation: [",
