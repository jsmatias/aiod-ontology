...
NumericMatrix calc_cooccurrence_C(const int n, const int m, const int i, int r, const List& reldb_l, int maxsize=1000) {
    // ChatGPT (modified): Overall, the function is performing co-occurrence analysis between the specified keyword (i)
    // and all other keywords in the reldb_l, and it stores the results in a matrix. 
    // The co-occurrence count represents the number of common entities between 
    // the specified keyword and each other keyword.
