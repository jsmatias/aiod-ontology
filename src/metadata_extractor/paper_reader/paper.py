...
class Paper:
    ...
    def _get_doi_from_file_name(self) -> str:
        ...
        filename = ".".join(self.file_name.split(".")[:-1])
    ...
    def _extract_abstract(self):
        ...
        if len(abstract_groups) > 1:
            ...
            raise Exception(msg)
        return abstract_groups[0].strip()
    ...
    def extract_topics_from_dbpedia(self):
        if not self.abstract:
            self.dbpedia_topics = []
        try:
            res_dict = dbpedia.api.get_annotate(self.abstract)
            self.dbpedia_topics = (
    ...
    def load_pdf(self):
        f"""Loads the PDF content from current file {self.full_path}"""
        self._pdf_info = self.extract_pdf_info()
        self._raw_text = self.extract_pdf_text()
        self.abstract = self._extract_abstract()
        self.keywords = self._extract_keywords_from_pdf_content()
    ...
    def extract_acm_topics(self):
        ...
        self.acm_topics = topics

    def extract_keywords_from_pdf_metadata(self):
    ...
    def load(self):
        ...
        if self.doi:
            ...
            self.acm_topics = self.extract_topics()
